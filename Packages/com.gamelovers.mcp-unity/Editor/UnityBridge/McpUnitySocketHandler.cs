...
namespace McpUnity.Unity
{
    ...
    public class McpUnitySocketHandler : WebSocketBehavior
    {
        ...
        {
            var message = e?.Message ?? "Unknown WebSocket error";
            if (ShouldDowngradeTransportError(message))
            {
                McpLogger.LogWarning($"WebSocket transport notice: {message}");
                return;
            }

            McpLogger.LogError($"WebSocket error: {message}");
        }

        private static bool ShouldDowngradeTransportError(string message)
        {
            ...
            if (normalized.Contains("closing the connection")
                || normalized.Contains("sending data"))
            {
                return true;
            }
